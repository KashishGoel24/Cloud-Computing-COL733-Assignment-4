...
class BayouServer(Server):
  ...
  def _process_req(self, msg: JsonMessage) -> JsonMessage:
    ...
    try:
      match req_type:
        ...
        case _:
          # # print("sending for prcess app req")
  ...
  def __send_anti_entropy(self, peer: str) -> tuple[list[LogEntry], SortedList[LogEntry]]:
    # print(self.name,"printing the self.c and self.f vectors we are sending throught the anti entropy request", self.storage.c, self.storage.f)
    response = self._connection_stub.send(self.name, peer, AntiEntropyReq(self.storage.c, self.storage.f).to_msg())
    # print(self.name, "the response from self.ani entropy", response)
    ...
    while True:
      for peer in self._peers:
        if peer.name != self.name:
          try:
            # print(self.name,"now sending anti entropy")
            committed, tentative = self.__send_anti_entropy(peer.name)
            # print(self.name, "going yo apply after anti entropy", committed, tentative)
